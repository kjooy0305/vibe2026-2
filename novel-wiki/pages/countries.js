...
window.Pages.countries = {
  ...
  _openForm: async function(container, item, wid, all) {
    ...
    const iconPool = (await DB.getSetting('iconList_country', null)) || this.ICONS;
    ...
    // Icon picker
    setTimeout(() => {
      const countryPicker = document.getElementById('iconGrid');
      const countryDisplay = document.getElementById('iconDisplay');
      let sel = currentIcon;
      if (countryDisplay) countryDisplay.dataset.icon = sel;
      countryPicker?.querySelectorAll('.icon-pick-btn').forEach(btn => {
        btn.addEventListener('click', () => {
          sel = btn.dataset.icon;
          if (countryDisplay) { countryDisplay.textContent = sel; countryDisplay.dataset.icon = sel; }
          countryPicker.querySelectorAll('.icon-pick-btn').forEach(b => b.style.borderColor = 'transparent');
